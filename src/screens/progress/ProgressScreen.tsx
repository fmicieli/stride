...
export function ProgressScreen() {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}><Text style={styles.headerTitle}>Tu progreso</Text></View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {plan && (
          <View style={styles.planProgress}>
            ...
            <Text style={styles.planProgressLabel}>{completedWeeks} de {plan.totalWeeks} semanas · {Math.round(progress * 100)}%</Text>
          </View>
        )}
              ...
              {recentSessions.map((s) => (
                <View key={s.id} style={styles.sessionCard}>
                  ...
                  <View style={styles.sessionStatsRow}>
                    ...
                    <Text style={styles.statDot}>·</Text>
                    <Text style={styles.sessionStat}>{s.pace > 0 ? formatPace(s.pace) + '/km' : '—'}</Text>
                  </View>
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  content: { paddingHorizontal: spacing[4], paddingTop: spacing[5], paddingBottom: spacing[10], gap: spacing[5] },
  summaryCard: { backgroundColor: colors.surfaceMuted, borderRadius: radius.sm, padding: spacing[4] },
  summaryRow: { flexDirection: 'row', alignItems: 'center' },
  summaryItem: { flex: 1, alignItems: 'center' },
  summaryNumber: { fontFamily: 'PlusJakartaSans-Bold', fontSize: 32, color: colors.ink[900], marginBottom: 4 },
  summaryLabel: { fontFamily: 'PlusJakartaSans', fontSize: 12, color: colors.ink[400], textAlign: 'center' },
  summaryDivider: { width: 1, height: 48, backgroundColor: colors.borderDefault },
  planProgress: { gap: 8 },
  sectionTitle: { fontFamily: 'PlusJakartaSans-SemiBold', fontSize: 16, color: colors.ink[900] },
  planProgressLabel: { fontFamily: 'PlusJakartaSans', fontSize: 12, color: colors.ink[400] },
  statsRow: { flexDirection: 'row', gap: spacing[3] },
  statCard: { flex: 1, backgroundColor: colors.surfaceMuted, borderRadius: radius.sm, padding: spacing[4] },
  statValue: { fontFamily: 'PlusJakartaSans-Bold', fontSize: 24, color: colors.ink[900], marginBottom: 4 },
  statUnit: { fontFamily: 'PlusJakartaSans', fontSize: 12, color: colors.ink[400] },
  historySection: { gap: spacing[3] },
  historyHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  verTodo: { fontFamily: 'PlusJakartaSans', fontSize: 14, color: colors.brand[500] },
  sessionsList: { gap: 8 },
  sessionCard: { backgroundColor: colors.surfaceMuted, borderRadius: radius.sm, padding: spacing[4], gap: 4 },
  ...
  sessionStatsRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 4 },
  sessionStat: { fontFamily: 'PlusJakartaSans', fontSize: 14, color: colors.ink[500] },
